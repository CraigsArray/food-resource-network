...
function PostAttachment({ url, title }) {
  ...
  if (!type) return null
  if (type === 'image') {
    return <img src={url} alt={title} style={{ width: '100%', height: 'auto', display: 'block', borderRadius: 8, marginBottom: '0.625rem' }} loading="lazy" />
  }
  ...
  return (
    <a href={url} target="_blank" rel="noopener noreferrer" style={{
      ...
      padding: '0.625rem 0.875rem', borderRadius: 8, marginBottom: '0.625rem',
      ...
      {isPdf ? (
        <div style={{ width: 34, height: 34, borderRadius: 7, background: '#e53935', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
          <span style={{ color: 'white', fontSize: '0.55rem', fontWeight: 900, letterSpacing: '-0.5px' }}>PDF</span>
        </div>
      ) : (
        <div style={{ width: 34, height: 34, borderRadius: 7, background: 'var(--color-bg-medium)', border: '1px solid var(--color-border)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        <div style={{ fontSize: '0.8rem', fontWeight: 600, color: 'var(--color-text-primary)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{filename}</div>
        <div style={{ fontSize: '0.72rem', color: 'var(--color-text-muted)' }}>{isPdf ? 'PDF Document' : 'File'} · Click to open</div>
      </div>
      <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" style={{ color: 'var(--color-text-muted)', flexShrink: 0 }}>
                            ...
                            {isOpen && (
                              <div style={{ marginTop: '0.875rem', paddingTop: '0.875rem', borderTop: '1px solid var(--color-border)' }}>
                                ...
                                {(() => {
                                  ...
                                  if ((isFsd || isSdfb) && post.image_url) {
                                    ...
                                    return (
                                      <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', marginBottom: '0.625rem', padding: '0.5rem 0.75rem', background: 'var(--color-surface)', borderRadius: 8, border: '1px solid var(--color-border)' }}>
                                        <img src={post.image_url} alt={name} style={{ height: 'auto', width: 'auto', maxHeight: 40, maxWidth: 40, objectFit: 'contain', flexShrink: 0 }} />
                                        <div>
                                  ...
                                  return <PostAttachment url={post.image_url} title={post.title} />
                                })()}
                                {post.tags?.length > 0 && (
